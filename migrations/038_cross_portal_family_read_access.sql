...
-- Revoke recipient-scoped messaging immediately when the underlying
-- authorization is removed. Historical participation alone is never
-- sufficient to keep reading or creating a family conversation.
...
CREATE POLICY message_threads_read ON message_threads
    ...
    USING (
        ...
        AND (
            ...
            OR app_user_authorized_for_recipient(
                NULLIF(current_setting('app.current_user_id', true), '')::uuid,
                message_threads.organization_id,
                message_threads.care_recipient_id
            )
        )
        OR app_is_superadmin()
    );

DROP POLICY IF EXISTS message_threads_create ON message_threads;
        ...
        AND (
            (shifts.care_recipient_id IS NOT NULL AND EXISTS (
                ...
                WHERE fr.care_recipient_id = shifts.care_recipient_id
                  ...
                  AND fr.status = 'active'
                  AND app_user_authorized_for_recipient(
                      fr.user_id, fr.organization_id, fr.care_recipient_id
                  )
            ))
